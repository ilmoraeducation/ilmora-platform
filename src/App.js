...
function PublicRoute({ children }) {
  const { currentUser, isAdmin } = useAuth();
  if (currentUser) return <Navigate to={isAdmin ? '/admin' : '/student'} replace />;
  return children;
}
  ...
  return (
    <Suspense fallback={<LoadingScreen />}>
      <Routes>
        ...
        <Route path="/student/*" element={<ProtectedRoute adminOnly={false}><StudentDashboard /></ProtectedRoute>} />
    ...
    <Router>
      <AuthProvider>
        ...
        <Toaster position="top-right" toastOptions={{
          style: { background:'#1a1a2e', color:'#fff', border:'1px solid rgba(201,168,76,0.3)', borderRadius:'12px', fontFamily:'Montserrat, sans-serif' },
